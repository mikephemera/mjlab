...
@pytest.fixture(scope="module")
def xiaoli_velocity_task_ids(velocity_task_ids: list[str]) -> list[str]:
  """Get all Xiaoli velocity task IDs."""
  return [t for t in velocity_task_ids if "Xiaoli" in t]
...
def test_go2_velocity_has_required_sensors(go2_velocity_task_ids: list[str]) -> None:
  """Go2 velocity tasks should have feet/ground and nonfoot/ground sensors."""
...
def test_xiaoli_velocity_has_required_sensors(
  xiaoli_velocity_task_ids: list[str],
) -> None:
  """Xiaoli velocity tasks should have feet/ground and nonfoot/ground sensors."""
  for task_id in xiaoli_velocity_task_ids:
    cfg = load_env_cfg(task_id)
...
def test_xiaoli_velocity_has_correct_action_scale(
  xiaoli_velocity_task_ids: list[str],
) -> None:
  """Xiaoli velocity tasks should use XIAOLI_ACTION_SCALE."""
  for task_id in xiaoli_velocity_task_ids:
    cfg = load_env_cfg(task_id)
    ...
    assert joint_pos_action.scale == XIAOLI_ACTION_SCALE, (
      f"Task {task_id} action scale mismatch, expected XIAOLI_ACTION_SCALE"
    )
